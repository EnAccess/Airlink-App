...
class _DeviceDetailsPageState extends State<DeviceDetailsPage> {
  ...
  Widget build(BuildContext context) {
    widget.model?.device!.connect(autoConnect: false);

    return WillPopScope(
      onWillPop: () async {
        widget.model?.device!.disconnect();
        return true;
      },
      child: Scaffold(
        appBar: AppBar(
          title: Text('${widget.model?.device?.name}'),
          actions: [
            StreamBuilder<BluetoothDeviceState>(
                stream: widget.model?.device!.state,
      ...
      children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int pst =
                            int.parse(widget.model!.pst.toString());
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            await BleServices().authorizeDevice(
                                context, widget.model!.did.toString());
                          },
        ...
        StreamBuilder<BluetoothDeviceState>(
            ...
            builder: (c, snapshot) {
              if (snapshot.data == BluetoothDeviceState.connected) {
                return Property(
                  bluetoothDevice: widget.model?.device,
                  context: context,
                );
              }
              return const Center(
  ...
  loadAuthTkn() async {
    final authTkn =
        await storage.read(key: 'D_${widget.model?.did.toString()}') ?? '';
  ...
  writeValuesToDevice(String deviceID) async {
    ...
    BluetoothDevice? bluetoothDevice = widget.model?.device;
  ...
  postDeviceDataToServer() async {
    ...
    TimeSeriesData timeSeriesData =
        TimeSeriesData(did: widget.model!.did, json: timeseriesData.toString());
    ...
    var result =
        await DeviceDatabase.instance.readTimeSeriesData(widget.model!.did);
    if (result != null) {
      //get device UUID
      final deviceUUID = await storage.read(key: 'deviceUUID') ?? '';
  ...
  _enterPaygToken() async {
    ...
    if (_snFormKey.currentState!.validate()) {
      ...
      //show loading dialog
      AlertWidgets.showLoading(context, 'Loading...', 'Sending token to device, please wait.');
      ...
      BluetoothDevice? bluetoothDevice = widget.model?.device;
      try {
        await BleServices().writeToCharacteristic(context, bluetoothDevice!, epochObj).then((value) async => await BleServices().writeToCharacteristic(context, bluetoothDevice, tknObj));
      } catch (e) {
        if (kDebugMode) {
          print('error: $e');
        }
      ...
      AlertWidgets.hideLoading(context);
      AlertWidgets.showAlert(context, AlertStatus.success, 'Success!', 'Token sent successfully.');
    }
