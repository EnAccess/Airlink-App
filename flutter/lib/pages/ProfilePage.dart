...
class _ProfilePageState extends State<ProfilePage> {
  ...
  _saveDetails() async {
    if (_formKey.currentState!.validate()) {
      ...
      await storage.write(key: 'deviceProfileId', value: deviceProfileIdController.text);
      await storage.write(key: 'gatewayProfileId', value: gatewayProfileIdController.text);

      AlertWidgets.showAlert(context, AlertStatus.success, 'Success!', 'Data saved.');
    }
  ...
  _provisionGateway() async {
    ...
    //show loading dialog
    AlertWidgets.showLoading(context, 'Provisioning', 'Provisioning gateway, please wait...');
    ...
    if (_formKey.currentState!.validate()) {
      ...
      //provision gateway
      http.Response response = await AirLinkServer().provisionDevice('DEVICE_ID: ${_androidDeviceId.toUpperCase()}', 'gateway');

      try {
        final serverResponse = serverResponseFromJson(response.body);
        AlertWidgets.hideLoading(context);
        AlertWidgets.showAlert(context, AlertStatus.error, 'Error!', serverResponse.message);
      }
      catch (e) {
        AlertWidgets.hideLoading(context);
        AlertWidgets.showAlert(context, AlertStatus.success, 'Success!', 'Gateway provisioned successfully');
      }
